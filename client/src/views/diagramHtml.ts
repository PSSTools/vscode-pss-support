...
/**
 * Serialize a graph for embedding inside an inline `<script>` block.
 *
 * `JSON.stringify` alone is not safe here: it leaves `<` untouched, so a node
 * label containing `</script>` would terminate the script element early and the
 * remainder would be parsed as HTML. Labels are built from identifiers and
 * expression text in the user's .pss files, which is not trusted input to this
 * webview. Escaping `<`, `>` and `&` as unicode escapes keeps the value a valid
 * JavaScript string literal while making it inert to the HTML parser.
 */
export function embedGraphJson(graph: DiagramGraph): string {
...
/**
 * Build the full webview document for an activity diagram.
 *
 * Pure: no `vscode` API access, so it can be rendered and asserted on in a
 * plain unit test. ActivityDiagramPanel is left with only the panel lifecycle.
 */
export function renderDiagramHtml(graph: DiagramGraph): string {
  const graphJson = embedGraphJson(graph);
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
